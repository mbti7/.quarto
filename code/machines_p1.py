...
SWITCH_POINT = 8   # 말의 개수에 따라 알고리즘 전환
...
# Variables
PLAYER = 1
isFirst = False # P1이므로 P2가 말을 선택하며 게임시작

# Minimax용 DP 메모이제이션 테이블
DP_table = {}
...
class MCTS:
    def __init__(self, exploration_weight=1):
        self.Q = defaultdict(int)  # 각 노드의 누적 보상값
        self.N = defaultdict(int)  # 각 노드의 방문 횟수
        self.children = dict()  # 각 노드의 자식 노드 목록
    ...
    def choose(self, node):
        ...
        def score(n):
            if self.N[n] == 0:
                return float("-inf")  # 한번도 시도되지 않은 노드는 제외
            return self.Q[n] / self.N[n]  # 평균 승률
    ...
    def _select(self, node):
        ...
        while True:
            ...
            # UCT값에 따라 가장 우수한 자식 노드 선택
            node = self._uct_select(node)

    def _expand(self, node):
        if node in self.children:
            return  # 이미 확정된 노드이면 무시
    ...
    def blocks_opponent_win(self):
        ...
        for (row, col) in self.board_state.available_places:
            ...
            if check_win(tmp_board, row, col):
                return False  # 상대에게 승리 수를 허용(패배)
        return True #상대에게 승리 수가 없음
...
class Board:
    def __init__(self, board, player, selected_piece, available_places, available_pieces):        
        self.__board = copy.deepcopy(board)    #게임 보드
        self.player = player                   #현재 플레이어
        self.selected_piece = selected_piece   #현재 선택된 조각
        self.available_places = copy.deepcopy(available_places) # 말이 놓일 수 있는 위치(빈칸)
        self.available_pieces = copy.deepcopy(available_pieces) # 남아있는 말
        self.last_move = None                  #마지막으로 놓인 말의 위치
    ...
    def place(self, row, col):
        ...
        self.available_places.remove((row, col))                    # 놓인 칸 제거
        self.selected_piece = None                                  # 배치 완료
        self.last_move = (row, col)                                 # 말 기록
